    ...
    >
      <div className="max-w-6xl mx-auto">
        <div className="grid md:grid-cols-12 gap-10">
          ...
          <form onSubmit={handleSubmit} className="md:col-span-7 space-y-6 md:pl-10 md:border-l border-ink/10">
            <div>
              <label className="font-mono text-xs uppercase tracking-[0.2em] text-muted block mb-2">Name</label>
              <input name="name" type="text" required disabled={status === "sending"} className="w-full bg-transparent border-b border-ink/30 py-3 text-lg font-serif focus:outline-none focus:border-accent transition-colors" />
            </div>

            <div>
              <label className="font-mono text-xs uppercase tracking-[0.2em] text-muted block mb-2">Email</label>
              <input name="email" type="email" required disabled={status === "sending"} className="w-full bg-transparent border-b border-ink/30 py-3 text-lg font-serif focus:outline-none focus:border-accent transition-colors" />
            </div>

            <div>
              <label className="font-mono text-xs uppercase tracking-[0.2em] text-muted block mb-2">Message</label>
              <textarea name="message" required rows={4} disabled={status === "sending"} className="w-full bg-transparent border-b border-ink/30 py-3 text-lg font-serif focus:outline-none focus:border-accent transition-colors resize-none" />
            </div>

            <div className="flex items-center gap-6 pt-4">
              <button type="submit" disabled={status === "sending"} className="font-mono text-sm uppercase tracking-wider px-8 py-3 bg-ink text-paper rounded-full hover:bg-accent transition-colors duration-300 disabled:opacity-50">
              ...
              {status === "error" && (
                <span className="text-sm text-red-700 font-mono">{errorMsg}</span>
              )}
            </div>
          </form>
        </div>
